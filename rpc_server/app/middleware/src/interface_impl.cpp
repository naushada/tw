...
int app::handle_read(std::int32_t handle, const std::string& in) {
  ...
  if(!rsp.empty()) {
    ...
    if(rpc_name.length() > 0 && (!rpc_name.compare("Subscribe"))) {
      ...
      rsp.ParseFromString(protobuf_bytes);
      std::cout << "Successfully parsed gNMI SubscribeResponse:" << std::endl;
      // You can now access fields like this:
      if(rsp.response_case() == gnmi::SubscribeResponse::kUpdate) {
        std::cout <<"gnmi::SubscribeResponse::kUpdate" << std::endl;
        if(rsp.has_update()) {
          const auto& notif = rsp.update();
          // Build the full path prefix string for clarity
          std::string prefix_str;
          for(const auto& elem : notif.prefix().elem()) {
            prefix_str += "/" + elem.name();
          }
          std::cout << "Prefix: " << prefix_str << std::endl;

          for (const auto& updt : notif.update()) {
            // Accessing nested fields, e.g., path elements
            std::cout << "  Path elements: ";
            for (const auto& elem : updt.path().elem()) {
              std::cout << "/" << elem.name();
              // Handle keys if necessary
            }
            std::cout << std::endl;
